...
export default function NotificationsDemo() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError("Нужен вход, чтобы увидеть реальные уведомления. Показаны демо-пустышки.");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const markAll = async () => {
    try {
      ...
      setError("Не удалось отметить прочитанными (нужен вход)");
    }
  };
  ...
  const markOne = async (id: string) => {
    try {
      ...
      setError("Не удалось обновить уведомление (нужен вход)");
    }
  };
      ...
      <div className="container space-y-8 py-12">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold">События профиля</h1>
            <p className="text-white/70">Просмотры профиля, добавление в шортлист, модерация.</p>
        ...
        {items.length === 0 && !loading && (
          <div className="card text-white/70">Уведомлений нет (или требуется вход).</div>
        )}
