...
export default function ScoutWorkingDetailPage() {
    ...
    <main className="min-h-screen bg-secondary">
      <div className="container space-y-8 py-12">
        <div className="flex items-center justify-between">
          <div>
            <p className="pill mb-2">Скаут • Карточка игрока</p>
            <h1 className="text-3xl font-bold">Рабочая карточка</h1>
            <p className="text-white/70">Редактируемая копия агентской карточки игрока.</p>
            {error && <p className="text-sm text-amber-300">{error}</p>}
            {message && <p className="text-sm text-emerald-300">{message}</p>}
            {loading && <p className="text-sm text-white/60">Загрузка...</p>}
          </div>
          <div className="flex gap-3">
            <Link href="/app/scout/working" className="ghost-btn">
              К списку
            </Link>
            {card && (
              <Link href={`/app/scout/players/${card.playerId}`} className="ghost-btn">
                Профиль игрока
              </Link>
            )}
          </div>
