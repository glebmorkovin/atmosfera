...
export default function ScoutSearchRealPage() {
  ...
  const loadPlayers = async () => {
    ...
    try {
      ...
      const data = await apiFetch<{ data: PlayerCard[] }>(`/players/search?${query.toString()}`);
  ...
  useEffect(() => {
    ...
    const loadRefs = async () => {
      try {
        const [leaguesRes, clubsRes] = await Promise.all([apiFetch<RefOption[]>("/refs/leagues"), apiFetch<RefOption[]>("/refs/clubs")]);
            ...
            <div key={p.id} className="card space-y-2">
              ...
              <div className="flex items-center justify-between text-xs text-white/60">
                ...
                <div className="flex gap-2">
                  <Link href={`/scout/player/${p.id}`} className="text-primary">
                    Профиль
                  </Link>
                  <Link href="/scout/shortlists" className="text-white/60 hover:text-primary">
                    Шортлисты
                  </Link>
                  {targetShortlist && (
                    <button
