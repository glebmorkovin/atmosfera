...
test("scout search opens profile", async ({ page }) => {
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/users/me" && request.method() === "GET") {
      return fulfillJson(route, 200, { role: "SCOUT" });
    }
  ...
  await page.fill('input[type="email"]', "scout@example.com");
  await page.fill('input[type="password"]', "password123");
  await page.click('button[type="submit"]');
  await page.waitForURL(/\/app\/scout\/dashboard/);

  await page.goto("/app/scout/search");
  const profileLink = page.locator('a[href="/app/scout/players/player-1"]');
  await expect(profileLink).toBeVisible();
  await profileLink.click();
  await expect(page).toHaveURL(/\/app\/scout\/players\/player-1/);
